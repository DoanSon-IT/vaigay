...
public class GoogleOAuth2LoginSuccessHandler implements AuthenticationSuccessHandler {
    ...
    public void onAuthenticationSuccess(HttpServletRequest request, HttpServletResponse response, Authentication authentication) throws IOException {
        ...
        if (!(principal instanceof CustomOidcUser oidcUser)) {
            response.sendError(HttpServletResponse.SC_INTERNAL_SERVER_ERROR, "Invalid OIDC user type");
            return;
        }

        String email = oidcUser.getEmail();
        if (email == null) {
            throw new IllegalStateException("Email from Google is null");
        }

        Optional<User> userOpt = userRepository.findByEmail(email);
        User user = userOpt.orElseGet(() -> {
            User newUser = User.oauthUser(oidcUser, AuthProvider.GOOGLE);
